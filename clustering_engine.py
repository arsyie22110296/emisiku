...
class EmissionClusterAnalyzer:
    ...
    def recommend_optimal_k(self, df: pd.DataFrame, max_k: int = 10, feature_cols: list = None) -> dict:
        """
        Auto-recommend K optimal berdasarkan kombinasi Elbow Method + Silhouette Score

        Logic:
        1. Jalankan Elbow method untuk rekomendasi awal
        2. Hitung Silhouette Score untuk setiap K (2-max_k)
        3. Combine: Elbow point + Silhouette tertinggi → final recommendation
        4. Return dengan confidence score

        Args:
            df: DataFrame dengan data emisi
            max_k: Maximum K untuk dievaluasi
            feature_cols: Kolom fitur untuk clustering

        Returns:
            dict dengan:
            - 'recommended_k': Rekomendasi jumlah cluster optimal
            - 'confidence': Confidence score (0-1)
            - 'reasoning': Penjelasan mengapa K tersebut direkomendasikan
            - 'metrics_by_k': Tabel metrik untuk setiap K
        """
        # Siapkan features
        self.prepare_features(df, feature_cols)

        # 1. Hitung Elbow Method
        elbow_result = self.find_optimal_k(df, max_k, feature_cols)
        elbow_k = elbow_result['optimal_k']
        ...
        for k in range(2, max_k + 1):
            try:
                ...
                clusters_temp = kmeans_temp.fit_predict(self.scaled_data)

                # Calculate metrics
                sil_score = silhouette_score(self.scaled_data, clusters_temp)
                ...
                metrics_by_k[k] = {'silhouette': 0, 'is_elbow': (k == elbow_k)}

        # 3. Combine rekomendasi
        # Prioritas: Silhouette score tinggi + Elbow point
        recommended_k = elbow_k
        confidence = 0.5
        reasoning = "Default elbow point"

        # Jika Elbow point memiliki Silhouette score yang bagus (> 0.5)
        if metrics_by_k.get(elbow_k, {}).get('silhouette', 0) > 0.5:
            recommended_k = elbow_k
            confidence = min(0.95, 0.5 + metrics_by_k[elbow_k]['silhouette'])
            reasoning = f"Elbow point (K={elbow_k}) dengan Silhouette Score {metrics_by_k[elbow_k]['silhouette']:.3f}"
        # Jika best silhouette score cukup bagus
        elif best_silhouette_score > 0.5:
            recommended_k = best_silhouette_k
            confidence = min(0.95, best_silhouette_score)
            reasoning = f"Tertinggi Silhouette Score (K={best_silhouette_k}) dengan score {best_silhouette_score:.3f}"
        # Fallback ke default 3
        else:
            recommended_k = 3
            confidence = 0.6
            reasoning = "Default K=3 (Low, Medium, High) - untuk general use case"
