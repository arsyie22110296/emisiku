...
def plot_emission_trend(df: pd.DataFrame):
    ...
    # 🔥 PERBAIKAN TOTAL: Buat copy dan pastikan kolom tanggal ada
    df_temp = df.copy()
    
    # CEK: Apakah kolom tanggal ada?
    if 'tanggal' not in df_temp.columns:
        # Buat kolom tanggal dari index
        df_temp['tanggal'] = df_temp.index.astype(str)
    else:
        # Coba konversi ke datetime jika perlu
        try:
            # Cek apakah kolom tanggal bisa dikonversi ke datetime
            df_temp['tanggal'] = pd.to_datetime(df_temp['tanggal'])
        except:
            # Jika gagal, biarkan sebagai string
            df_temp['tanggal'] = df_temp['tanggal'].astype(str)
...
def plot_cluster_scatter(df: pd.DataFrame):
    ...
    if 'tanggal' in df.columns:
        try:
            hover_text = pd.to_datetime(df['tanggal']).dt.strftime('%Y-%m-%d')
        except:
    ...
    for cluster_label in df['cluster_label'].unique():
        ...
        # Ambil hover text untuk cluster ini
        cluster_hover = hover_text[df_cluster.index] if isinstance(hover_text, pd.Series) else hover_text
...
def plot_weekly_pattern(df: pd.DataFrame):
    ...
    # Buat copy dan konversi tanggal
    df_temp = df.copy()
    try:
...
def plot_forecast(forecast_df: pd.DataFrame):
    """
    Plot forecast emisi 30 hari dengan confidence interval

    Args:
        forecast_df: DataFrame dari forecasting_linear dengan kolom:
                    - tanggal/ds: tanggal forecast
    ...
    if 'tanggal' not in df_plot.columns:
        df_plot['tanggal'] = df_plot.get('ds', pd.date_range(start='today', periods=len(df_plot)))
    ...
    fig.update_layout(
        title='🔮 Prediksi Emisi Karbon 30 Hari Ke Depan',
        xaxis_title='Tanggal',
        yaxis_title='Emisi (kg CO₂)',
        hovermode='x unified',
        height=450,
...
def plot_emission_trend_with_anomalies(df: pd.DataFrame):
    """
    Plot tren emisi dengan highlight anomali

    Args:
        df: DataFrame dengan kolom:
           - tanggal: tanggal data
           - total_emisi_kgco2: nilai emisi
           - is_anomaly: boolean flag untuk anomali
           - anomaly_score: score severity (optional)
    """
    df_plot = df.copy()

    # Ensure datetime
    if 'tanggal' in df_plot.columns:
        df_plot['tanggal'] = pd.to_datetime(df_plot['tanggal'])
    else:
        df_plot['tanggal'] = pd.date_range(start='today', periods=len(df_plot))

    # Ensure anomaly columns exist
    if 'is_anomaly' not in df_plot.columns:
        df_plot['is_anomaly'] = False
    if 'anomaly_score' not in df_plot.columns:
        df_plot['anomaly_score'] = 0

    fig = go.Figure()

    # Separate normal and anomaly data
    df_normal = df_plot[~df_plot['is_anomaly']]
    df_anomaly = df_plot[df_plot['is_anomaly']]

    # Plot normal points
    fig.add_trace(go.Scatter(
        x=df_normal['tanggal'],
        y=df_normal['total_emisi_kgco2'],
        mode='lines+markers',
        name='Normal',
        line=dict(color='steelblue', width=2),
        marker=dict(size=8, color='steelblue'),
        hovertemplate='<b>%{x|%Y-%m-%d}</b><br>Emisi: %{y:.0f} kg CO₂<extra></extra>'
    ))

    # Plot anomaly points
    if len(df_anomaly) > 0:
        fig.add_trace(go.Scatter(
            x=df_anomaly['tanggal'],
            y=df_anomaly['total_emisi_kgco2'],
            mode='markers',
            ...
            hovertemplate='<b>%{x|%Y-%m-%d}</b><br>Emisi: %{y:.0f} kg CO₂<br>⚠️ ANOMALI<extra></extra>'
        ))

    # Moving average line
    ma_7 = df_plot['total_emisi_kgco2'].rolling(window=7, min_periods=1).mean()
    fig.add_trace(go.Scatter(
        x=df_plot['tanggal'],
        y=ma_7,
        mode='lines',
        ...
        hovertemplate='<b>%{x|%Y-%m-%d}</b><br>MA-7: %{y:.0f} kg CO₂<extra></extra>'
    ))

    fig.update_layout(
        title='📈 Tren Emisi dengan Deteksi Anomali',
        xaxis_title='Periode Waktu',
        yaxis_title='Emisi (kg CO₂)',
        hovermode='x unified',
        height=500,
